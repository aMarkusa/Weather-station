...
#include "env_task.h"              // proximity functions
#include "gpio.h"
...
{
    struct env_db_cfg* db_cfg;

    struct gapm_profile_task_add_cmd *req = KE_MSG_ALLOC_DYN(GAPM_PROFILE_TASK_ADD_CMD,
                                                             TASK_GAPM,
                                                             TASK_APP,
                                                             gapm_profile_task_add_cmd,
                                                             sizeof(struct env_db_cfg));
    ...
    // Set parameters
    db_cfg = (struct env_db_cfg* ) req->param;
...
/*void app_env_alert_start(uint8_t lvl)
{
    alert_state.lvl = lvl;
...
void app_env_port_reinit(GPIO_PORT port, GPIO_PIN pin)
{
...
void default_env_alert_ind_handler(uint8_t conidx, uint8_t alert_lvl, uint8_t char_code)
{
